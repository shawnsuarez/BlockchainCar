...
// Meta Mask Account Mneumonic and Infura API Key
...
const deploy = async (vin, status) => {
	...
	console.log("VIN: ", await result.methods.vin().call());
	console.log("Status:", await result.methods.status().call());
};

//deploy(initialVin, initialStatus);
